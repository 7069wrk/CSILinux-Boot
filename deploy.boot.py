...
# CREDIT TO ORIGINAL AUTHOR!!! AWESOME WORK THAT I AM HACKING APART HERE
#   ___         _ ___  
#  |   \ ___ __| / __| ___ __   
#  | |) / -_) _` \__ \/ -_) _|
...
def banner():
   ...
   |___/\___\__,_|___/\___\__|  """,end="")
    ...
    print(f"   hacked up by {B}7069wrk{C} (theAnalyst)")
...
def install():
    ...
    print(f"\n{Y}(#){C} CSILinux GRUB theme has been successfully installed !!\n")

    distro = check_distro()
    if distro == "kali":  # checking if it's Kali Linux
        print(f"""{R}PS{C}: This is CSI LINUX; not Kali Linux.\n
    If you want to play with Kali - it wont happen here{C}""")
    exit()
...
def uninstall():
    ...
    ask = input(f"{B}(?){C} Remove CSILinux GRUB Theme (y/n)? {Y}[default = n]{C} : ")
    if ask.lower() != "y":
    ...
    print(f"\n{Y}(#){C} CSILinux GRUB Theme has been successfully removed !!\n")
    exit()
